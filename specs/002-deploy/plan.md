...
| Рабочий процесс: код по задаче | удаление S3 — T094 из фазы 1.5, перенос compose — T101 — ✅ |
...
`news_parser`: S3 полностью удалён (T094). Посты пишутся только в SQLite.

## Complexity Tracking

Не заполняется: нарушений конституции нет.

## Известная проблема: GitHub Actions не стартует в этом репозитории (03.09.2026)

Любой workflow, включая трёхстрочный `echo ok` в проверочной ветке, завершается `startup_failure` с пометкой `BuildFailed`. Файл валиден: `actionlint` и три бисект-варианта это подтвердили; статус GitHub Actions — operational. Причина на уровне аккаунта или приватного репозитория — вероятнее всего биллинг минут Actions для приватных репозиториев или ограничение аккаунта. Точный текст виден только владельцу на странице запуска и в https://github.com/settings/billing; API для этого требует scope `user`.

**Запасной путь**: `deploy/deploy.sh` — те же шесть шагов с рабочей машины, тем же ключом `~/.ssh/ai-gpr-deploy`. Когда Actions заработает, workflow подхватит следующий пуш без изменений.
