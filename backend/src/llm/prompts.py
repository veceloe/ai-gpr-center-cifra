...
def build_score_prompt(
    ...
    prompt_id = f"score_{'npa' if is_npa else 'news'}/v3"
...
"""
    ...
    # Пример ответа перечисляет ВСЕ критерии схемы. В версии v1 пример показывал
    # только первый, и модель его копировала: возвращала один-два балла вместо
    # шести, валидация отбраковывала ответ, материал оставался без оценки.
    # Дефект найден прогоном на эталоне заказчика — см. evals/README.md.
    scores_example = ", ".join(f'"{code}": 0' for code in scheme_cfg.codes)
    rationales_example = ", ".join(f'"{code}": "почему именно этот балл"' for code in scheme_cfg.codes)

    system = f"""Ты оцениваешь материал отраслевого мониторинга с точки зрения конкретной компании.

Выставь балл от 0 до 3 по каждому из критериев: {codes}. Только целые числа.
Критериев ровно {len(scheme_cfg.codes)}. Пропустить хотя бы один нельзя — ответ
без всех критериев будет отбракован, и материал останется без оценки.

Шкалы критериев — используй их буквально, не изобретай промежуточных значений:
{_scale_block(config, scheme)}

Нижняя граница шкалы «{scheme_cfg.relevance_criterion}». Ноль означает «связи нет вовсе» — материал
подпадает под явные признаки нерелевантности из профиля. Если связь есть, но
непрямая или общеотраслевая, это 1 балл, а не 0: так устроена шкала методики.
Прежде чем поставить 0, проверь каналы влияния, названные в самой шкале:
продукты, процессы компании, затраты, выручка, продуктовая стратегия, — а также
зоны риска и зоны роста из профиля. То, что компания не упомянута в материале
по имени, основанием для нуля не является.
...
Отвечай ТОЛЬКО валидным JSON. В scores и rationales должны быть все
{len(scheme_cfg.codes)} критерия, ровно с этими ключами:
{{"scores": {{{scores_example}}},
 "rationales": {{{rationales_example}}},
