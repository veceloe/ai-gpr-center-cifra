...
ACT_LIFECYCLE = Prompt(
    id="act_lifecycle/v1",
    system="""Ты определяешь текущую стадию жизненного цикла НПА по одному материалу.

Используй только материал типа "act". Тип материала уже задан backend/source flow и
не подлежит переопределению.

Стадии по порядку:
- "announcement" — анонс, подготовка или публикация карточки проекта без обсуждения
- "draft_discussion" — проект находится на общественном/публичном обсуждении
- "submitted" — законопроект или проект внесён в орган рассмотрения
- "readings" — идёт рассмотрение в чтениях или аналогичная процедура согласования
- "adopted" — акт принят, одобрен или подписан, но из текста не следует вступление в силу
- "in_force" — акт вступил/вступает в силу или начал действовать

Жёсткие правила:
- Верни stage_candidate = null, если в тексте нет явного основания для текущей стадии.
- evidence_quote — точная цитата из исходного текста, подтверждающая именно текущую
  стадию; без цитаты stage_candidate должен быть null.
- Не считай "опубликован для обсуждения" или "размещён проект" стадией "in_force".
- Не путай историческое описание старой стадии с текущим статусом материала.
- Не откатывай стадию назад: если материал явно описывает более ранний этап, верни
  эту стадию как наблюдение, а решение об откате всё равно принимает backend.
- Не определяй topic, item_type, оценку, индекс или итоговую категорию.

Отвечай ТОЛЬКО валидным JSON вида:
{"stage_candidate": "draft_discussion",
 "confidence": 0.86,
 "evidence_quote": "Проект проходит общественное обсуждение до 15 сентября",
 "rationale": "В цитате прямо указано общественное обсуждение проекта."}""",
)
...
def act_lifecycle_user_message(
    title: str,
    summary: str,
    text: str,
    url: str,
    ...
    return (
        f"Тип материала (задан источником): act\n"
        f"Текущая стадия досье в базе: {current_stage}\n"
        f"Идентификатор досье: {act_identifier or ''}\n"
        f"URL: {url}\n"
        f"Заголовок: {title}\n\n"
